...
function plugin_table_edit_action_edit($page, $from_page, $table_id, $row, $digest)
{
	...
	if ($row == -1) {
		...
		$body .= '<form action="' . get_script_uri() . '?cmd=' . PLUGIN_TABLE_EDIT_PLUGIN_NAME . '&mode=delete&from=' . rawurlencode($from_page) . '&table=' . $table_id . '&row=' . $row . '&page=' . rawurlencode($vars['page'])  . '&digest=' . $digest . '" method="post">';
